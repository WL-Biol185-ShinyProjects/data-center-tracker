...
This notebook is archived in `legacy/` because it is incomplete and not part of
the current Shiny app or ETL flow.
...
```{r}
# CLEANUP FUNCTION FOR ALL STATES


employed <- lapply(dataframes_list, function(table){
  if (substring(attr(table, "Series.Title"), 1 , 74) == "All Employees in Total Covered Total, all industries for All establishment" ){
   
    table$state <- attr(table, "State")
...
employed <- employed[!is.na(employed)]

all_states_employed <- do.call(rbind, employed)


establishment <- lapply(dataframes_list, function(estabCount){
  if(substring(attr(table, "Series.Title"), x, x) == "TITLE"){
    table$
